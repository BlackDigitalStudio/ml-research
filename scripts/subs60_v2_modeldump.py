#!/usr/bin/env python3
"""INTROSPECTION CAPTURE-RERUN (rev16) - byte-copy of protocol v2 + per-fold model dumps.
NOT a protocol script; outputs are models for anatomy, cells come from rev14 artifacts.
Original doc:
"""
_ORIG_DOC = """PROTOCOL v2 CANDIDATE (HD3 rev10 class-B; NOT frozen until the bridge run passes).
...
def run_fold(fi, trn, tst):
    ...
    if os.environ.get("MODEL_DUMP", "") == "1":
        import tempfile
        tag = os.environ.get("PFTAG", "")
        for nm, booster in (("A", A), ("Bg", Bg), ("Bf", Bf)):
            tmp = tempfile.mktemp(suffix=".json")
            booster.save_model(tmp)
            bk.blob(f"research_runs/{OUT_SUB}/MODELS{tag}_f{fi}_{nm}.json").upload_from_filename(tmp)
            os.remove(tmp)
        bk.blob(f"research_runs/{OUT_SUB}/MODELS{tag}_f{fi}_hp.json").upload_from_string(
