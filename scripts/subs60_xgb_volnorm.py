#!/usr/bin/env python3
"""f2 fix test: VOL-NORMALIZED features. Causal trailing per-feature z-score (each feature
normalized by the mean/std of the prior K days) -> feature magnitudes become vol-regime
invariant, removing the covariate shift that made the model capture alpha only when test vol
matched train vol (f2). Retrain A + B (gated & full) on normalized features; walk-forward
30s, ZERO fee, causal rolling deploy (noA & AxB) at 5/10 trades/day. Compares per-fold to the
non-normalized baseline (noA 30s t5 was annS +5.39, per-fold [-0.3,3.8,23.2,0.9,3.9,-4.3]).
...
CFGIDX, RHKEY = 1, "rH30"  # 30s horizon
BUDGETS = [5, 10]
...
# ---- causal trailing per-feature z-score (prior KNORM days) ----
print(f"[vol-norm] computing causal trailing z-score (K={KNORM}d) over {nfeat} feats ...", flush=True)
...
print(f"[vol-norm] done. Fn shape={Fn.shape}", flush=True)
...
print(f"[VOL-NORM 30s | ZERO fee | causal rolling] {len(FOLDS)} folds OOS={tot_days}d\n", flush=True)
...
print(f"  {'pol':>4} {'tgt':>4} {'trd/d':>6} {'EV/trd':>8} {'Shrp':>6} {'annS':>6} {'hit%':>6} {'tot/d':>7}  per-fold")
RES = {}
for tgt in BUDGETS:
    for pol in ("AxB", "noA"):
        pf = []
        ...
        RES[f"{pol}_t{tgt}"] = x
bk.blob("research_runs/maker_labels_h/VOLNORM_RESULT.json").upload_from_string(json.dumps(RES, default=float))
print("\n[saved] VOLNORM_RESULT.json | compare noA_t5 per-fold vs non-norm [-0.3,3.8,23.2,0.9,3.9,-4.3] (annS+5.39)", flush=True)
