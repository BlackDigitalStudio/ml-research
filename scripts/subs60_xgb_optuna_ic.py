...
print(f"[Optuna IC-objective on blanket vol-norm | trials={N_TRIALS}] Fn={Fn.shape}", flush=True)
...
for tgt in BUDGETS:
    for pol in ("AxB", "noA"):
        ...
        RES[f"{pol}_t{tgt}"] = x
bk.blob("research_runs/maker_labels_h/OPTUNA_IC_RESULT.json").upload_from_string(json.dumps(RES, default=float))
print("\n[saved] OPTUNA_IC_RESULT.json | frozen noA_t5 +2.42 ; AUC-Optuna noA_t5 +0.30", flush=True)
