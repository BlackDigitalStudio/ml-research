#!/usr/bin/env python3
"""RECORDER-EV v2 (rev18 ETH deploy): N_SEEDS env, DROP_COLS keep-slicing from the
bundle meta, per-seed scores saved (score_seeds) for per-seed tau seeding + harmony.
Base: the frozen v1 script.
Original doc: RECORDER-EV for the h150 deploy candidate: apply the 4-seed deploy_h150 ensemble to LIVE
...
SEEDS = list(range(int(os.environ.get('N_SEEDS', '4')))); BUN = []
for s in SEEDS:
...
KEEP = meta.get("keep_cols") or list(range(71))
print(f"[recev_h150v2] keep {len(KEEP)}/71 cols", flush=True)
...
for day in rec_days():
    ...
    sc_seeds = np.zeros((len(BUN), len(F)), np.float32)
    for bi, bd in enumerate(BUN):
        Fn = ((Fk - bd["mu"]) / bd["sd"]).astype(np.float32)
        pA = bd["A"].predict(xgb.DMatrix(Fn)); pBg = bd["Bg"].predict(xgb.DMatrix(Fn))
        ps = cdf(pA, bd["sA"]) * cdf(np.abs(pBg - 0.5), bd["sBg"])
        sc += ps; sc_seeds[bi] = ps.astype(np.float32); pbg_mean += pBg
    sc /= len(BUN); pbg_mean /= len(BUN); side = pbg_mean >= 0.5
    buf = io.BytesIO()
    np.savez(buf, score=sc.astype(np.float32), side=side, netl=PL.astype(np.float32), nets=PS.astype(np.float32), FL=FL, FS=FS, score_seeds=sc_seeds)
