#!/usr/bin/env python3
"""PER-FOLD Optuna HP tuning on the BLANKET vol-normalized features, DOGE 30s, ZERO fee.
Each fold: hold out the last 30 train days as sub-val; Optuna (TPE, N trials, on a sub-train
subsample, early-stop on sub-val AUC) tunes A (vol) and B (better-side) HP independently;
refit A / B-gated / B-full on the FULL train at the tuned HP; causal-rolling deploy noA & AxB at
5/10 trades/day. Honest walk-forward (val != test). Compare to frozen-HP blanket baseline (noA
annS +2.42). Reads research_runs/maker_labels_h/DOGE.npz.
"""
import io, json
import numpy as np
...
SUBVAL_D = 30; N_TRIALS = 20; TUNE_SUB = 150000; MAXR = 400; ESR = 30
CFGIDX, RHKEY = 1, "rH30"; BUDGETS = [5, 10]
BASE = {"objective": "binary:logistic", "tree_method": "hist", "nthread": 8, "seed": 0, "eval_metric": "auc"}
bk = storage.Client(project=PROJ).bucket(BUCKET)


def tune(Xst, yst, Xv, yv, wst=None, spw=None):
    if len(Xst) > TUNE_SUB:
        ix = np.random.RandomState(0).choice(len(Xst), TUNE_SUB, replace=False)
        Xst, yst = Xst[ix], yst[ix]; wst = wst[ix] if wst is not None else None
    dst = xgb.DMatrix(Xst, label=yst, weight=wst); dv = xgb.DMatrix(Xv, label=yv)

    def obj(t):
        hp = {"max_depth": t.suggest_int("max_depth", 3, 9),
        ...
        b = xgb.train(p, dst, num_boost_round=MAXR, evals=[(dv, "v")], early_stopping_rounds=ESR, verbose_eval=False)
...
# blanket vol-norm (causal trailing per-feature z-score)
...
print(f"[Optuna on blanket vol-norm | N_TRIALS={N_TRIALS}] Fn={Fn.shape}", flush=True)
...
for fi, (trn, tst) in enumerate(FOLDS):
    ...
    hpA, biA, aucA = tune(Fn[sub], yA[sub], Fn[val], yA[val], spw=spwA)
    ...
    fmask = (fl | fs)
    sb = sub & fmask; vb = val & fmask
    wcl = lambda mk: np.clip(wq[mk], 0, np.quantile(wq[mk][wq[mk] > 0], 0.99) if (wq[mk] > 0).any() else 1.0)
    hpB, biB, aucB = tune(Fn[sb], yB[sb], Fn[vb], yB[vb], wst=wcl(sb))
    gfull = trn & fmask; ggate = gate & fmask
    Bf = fith(hpB, biB, Fn[gfull], yB[gfull], w=wcl(gfull))
    Bg = fith(hpB, biB, Fn[ggate], yB[ggate], w=wcl(ggate))
    ...
    print(f"  fold{fi}: A val-AUC={aucA:.3f}(d{hpA['max_depth']},bi{biA}) B val-AUC={aucB:.3f}(d{hpB['max_depth']},bi{biB})", flush=True)
...
for tgt in BUDGETS:
    for pol in ("AxB", "noA"):
        ...
        RES[f"{pol}_t{tgt}"] = x
bk.blob("research_runs/maker_labels_h/OPTUNA_VOLNORM_RESULT.json").upload_from_string(json.dumps(RES, default=float))
print("\n[saved] OPTUNA_VOLNORM_RESULT.json | frozen-HP blanket baseline: noA_t5 annS+2.42", flush=True)
