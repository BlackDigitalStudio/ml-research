...
// EXIT-REASON instrumentation (probe build only). Index = ExitReason::id().
#[allow(clippy::declare_interior_mutable_const)]
static EXL: [AtomicU64; 12] = [AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0)];
#[allow(clippy::declare_interior_mutable_const)]
static EXS: [AtomicU64; 12] = [AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0), AtomicU64::new(0)];
...
/// cumulative opposing taker flow clears the queue; exit at the touch at the fill tick. If the forward
/// path ends before filling -> mark at the last touch (no fee). Returns (net_pnl_pct, filled:1/ranout:0).
fn simulate_pegged_exit(
    direction: SimDirection,
    fpx: f64,
    q_exit: f64,
    book_path: &[BookL1],
    ...
    let long = matches!(direction, SimDirection::Long);
    let sign = if long { 1.0 } else { -1.0 };
    let n = book_path.len().min(flow_path.len());
    if n == 0 || fpx <= 0.0 {
        return (0.0, 0);
    }
    let touch = |b: &BookL1| if long { b.ask } else { b.bid };
    let q0 = q_exit.max(0.0);
    let mut queue = q0;
    let mut post_px = touch(&book_path[0]);
    for t in 0..n {
        let cur = touch(&book_path[t]);
        if cur <= 0.0 {
            continue;
        }
        // adverse move: price ran away from our limit (long sell: ask fell; short buy: bid rose) ->
        // cancel & re-post at the new touch, last in the new queue.
        let adverse = if long { cur < post_px } else { cur > post_px };
        if adverse {
            post_px = cur;
            queue = q0;
        }
        let opp = if long { flow_path[t].buy_vol } else { flow_path[t].sell_vol };
        queue -= opp;
        if queue <= 0.0 {
            return (sign * (cur - fpx) / fpx * 100.0, 1);
        }
    }
    let cur = touch(&book_path[n - 1]);
    let cur = if cur > 0.0 { cur } else { book_path[n - 1].mid() };
    (sign * (cur - fpx) / fpx * 100.0, 0)
}
...
fn main() -> Result<()> {
    ...
    let per_sample: Vec<(Vec<(f64, f64)>, (u8, u8))> = (0..ns)
        ...
        .map(|i| {
            ...
            if use_book {
                ...
                // Resting-limit entry fill + pegged-exit queue (once per sample) when in maker mode.
                let (fill_l, fill_s, flowv, qex_l, qex_s) = if maker {
                    ...
                    let qex_s = entry_q_arr.as_ref().map(|q| q[[i, 0]]).unwrap_or(0.0) * a.exit_queue_mult;
                    (fl_e, fs_e, flowv, qex_l, qex_s)
                } else {
                    (None, None, Vec::new(), 0.0, 0.0)
                };
                if maker {
                ...
                for c in configs.iter() {
                    ...
                    let (l, s) = if maker {
                        // PEGGED maker exit (NO taker): hold `c.to` ticks from fill, then rest a close-limit
                        // pegged to the touch, ALWAYS LAST in queue, chasing the market until opposing flow
                        // clears the queue. P&L = (fill touch − fpx), 0 fee. EXL/EXS[7]=passive-fill, [8]=ran-out.
                        let _ = (&cfg, lat);
                        let lp = match fill_l {
                            Some((k, fpx)) => {
    ...
    // EXIT-REASON HISTOGRAM (probe). 7=TimeoutLimit(MAKER, comm_win), 8=TimeoutMarket(TAKER, comm_loss).
    let names = ["TpHit", "SlHit", "TrailSl1", "TrailSl2", "PartTp", "PartTrSl1", "PartTrSl2",
                 "PassiveFill(0fee)", "RanOut(marked)", "FastFillAdv", "FastFillSl", "NoFwd"];
    for (side, arr) in [("LONG", &EXL), ("SHORT", &EXS)] {
        let tot: u64 = arr.iter().map(|x| x.load(Ordering::Relaxed)).sum();
        eprintln!("EXIT-REASON [{}] filled={} (config-summed):", side, tot);
        for j in 0..12 {
            let c = arr[j].load(Ordering::Relaxed);
            if c > 0 {
                eprintln!("  {:22} {:>10} ({:.2}%)", names[j], c, 100.0 * c as f64 / tot.max(1) as f64);
            }
        }
    }
