#!/usr/bin/env python3
"""PROTOCOL v1 + FULL MODEL CAPTURE (MODELCAP rev1). Measurement path byte-unchanged.

= scripts/subs60_xgb_optuna_ic.py (the FROZEN v1 trainer, left untouched so every ledger
record that cites its md5 stays valid) + OUTPUT-ONLY capture. Nothing here touches the
data path, the RNG stream, the trial order or the training calls: every added line either
serialises an object that was already built, or writes a json. The frozen v1 saved only
per-fold composite scores (PERFOLD), so the WEIGHTS of every deployed cell were
unrecoverable - that is the gap this file closes.

Added, all output-only:
  MODEL_DUMP=1  final per-fold boosters A / Bg / Bf + the chosen HP  -> MODELS{tag}_...
  TRIAL_DUMP=1  EVERY search trial's booster + its params/objective  -> TRIALS{tag}_...
  OUT_SUB       artifact subdir (default LABELSUB, i.e. alongside the dataset)

BINDING GATE before any production use: a re-run must reproduce the stored PERFOLD
scores of an already-measured (symbol, seed) cell bit-exactly. If it does not, the
capture is NOT proven output-only and no model from it may be called "the deployed one".
...
MODEL_DUMP = os.environ.get("MODEL_DUMP", "1") == "1"
TRIAL_DUMP = os.environ.get("TRIAL_DUMP", "1") == "1"
PFTAG = os.environ.get("PFTAG", "")


def _put_booster(b, key):
    import tempfile
    tmp = tempfile.mktemp(suffix=".json")
    b.save_model(tmp)
    bk.blob(key).upload_from_filename(tmp)
    os.remove(tmp)


def _put_json(obj, key):
    bk.blob(key).upload_from_string(json.dumps(obj, default=float))
...
def tune_A(Xst, yst, Xv, yv, spw, fi=-1):
    ...
    def obj(t):
        ...
        t.set_user_attr("bi", int(b.best_iteration))
        if TRIAL_DUMP:   # output-only: b is already trained and is not touched again
            _put_booster(b, f"research_runs/{OUT_SUB}/TRIALS{PFTAG}_{SYM}_f{fi}_A_t{t.number}.json")
            log.append(dict(trial=int(t.number), params=hp, auc=float(b.best_score), bi=int(b.best_iteration)))
    ...
    if TRIAL_DUMP:
        _put_json(log, f"research_runs/{OUT_SUB}/TRIALS{PFTAG}_{SYM}_f{fi}_A_index.json")
    return st.best_params, int(st.best_trial.user_attrs["bi"]), float(st.best_value)


def tune_B_ic(Xst, yst, wst, Xv, pdiff_v, fi=-1):
    ...
    def obj(t):
        ...
        if pv.std() < 1e-9:
            ...
            ic = ic if np.isfinite(ic) else -1.0
        if TRIAL_DUMP:
            _put_booster(b, f"research_runs/{OUT_SUB}/TRIALS{PFTAG}_{SYM}_f{fi}_B_t{t.number}.json")
            log.append(dict(trial=int(t.number), params=dict(hp, num_boost_round=int(nr)), ic=float(ic)))
        return ic
    st = optuna.create_study(direction="maximize", sampler=optuna.samplers.TPESampler(seed=SEED)); st.optimize(obj, n_trials=N_TRIALS)
    if TRIAL_DUMP:
        _put_json(log, f"research_runs/{OUT_SUB}/TRIALS{PFTAG}_{SYM}_f{fi}_B_index.json")
...
# FOLD_ONLY=k: compute ONLY fold k. Folds share no mutable state and each builds its own
# seeded RandomState / optuna study (the same property that made FOLD_PAR parity-safe),
# so fold k's artifacts are identical to those of a full run. Used to buy the bit-parity
# gate for ~1/6 of the cost; the aggregate RES table of such a run is meaningless.
FOLD_ONLY = int(os.environ.get("FOLD_ONLY", "-1"))
...
def run_fold(fi, trn, tst):
    ...
    if MODEL_DUMP:   # output-only, after every prediction the fold needs has been made
        for nm, booster in (("A", A), ("Bg", Bg), ("Bf", Bf)):
            _put_booster(booster, f"research_runs/{OUT_SUB}/MODELS{PFTAG}_{SYM}_f{fi}_{nm}.json")
        _put_json(dict(hpA=hpA, biA=int(biA), aucA=float(aucA), hpB=hpB, biB=int(biB), icB=float(icB)),
                  f"research_runs/{OUT_SUB}/MODELS{PFTAG}_{SYM}_f{fi}_hp.json")
    return (axb_tr, axb_te, noa_tr, noa_te, day[tri], day[tei], pBg_te >= 0.5, pBf_te >= 0.5, fl[tei], fs[tei], netl[tei], nets[tei])


_JOBS = [(fi, trn, tst) for fi, (trn, tst) in enumerate(FOLDS)
         if FOLD_ONLY < 0 or fi == FOLD_ONLY]
if FOLD_ONLY >= 0:
    print(f"*** FOLD_ONLY={FOLD_ONLY}: parity probe, the aggregate table below is NOT a cell ***", flush=True)

if FOLD_PAR > 1:
    from concurrent.futures import ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=FOLD_PAR) as _ex:
        perfold = list(_ex.map(lambda a: run_fold(*a), _JOBS))
else:
    perfold = [run_fold(*a) for a in _JOBS]

if os.environ.get("SAVE_PF", "") == "1":   # capture-everything: per-fold test scores for offline selectivity work
    tag = os.environ.get("PFTAG", "")
    # key by the REAL fold index, which is not the list position under FOLD_ONLY
    for (fi, _t, _s), (axb_tr, axb_te, noa_tr, noa_te, dt, de, sBg, sBf, flt, fst, nlt, nst) in zip(_JOBS, perfold):
        pbuf = io.BytesIO()
