#!/usr/bin/env python3
"""PROTOCOL v2.1 + FULL TRIAL CAPTURE (MODELCAP rev1). Measurement path byte-unchanged.

= scripts/subs60_xgb_sobol_v2.py @ md5 2a435f32e2517479e0232260c85aad1a (the frozen v2.1
used for the ETH v2notod deploy cell and the BNB/LTC/SOL tier; left untouched on its own
branch so its md5 provenance stays valid) + OUTPUT-ONLY capture of every search trial's
booster. v2.1 already dumps the FINAL per-fold A/Bg/Bf; what it discards is the search
itself - 25 Sobol points x 2 studies x 6 folds per seed - which is exactly the population
needed to tell a seed-driven ensemble gain from an HP-landscape-driven one.

Added, all output-only: TRIAL_DUMP=1 -> TRIALS{tag}_{SYM}_f{fold}_{A|B}_t{trial}.json
plus a per-fold index json carrying params and the objective value of every trial.

BINDING GATE: a re-run must reproduce the stored PERFOLD scores AND the existing
MODELS_* boosters of an already-measured cell before anything from it is quoted.
...
def _trial_key(fi, study, t):
    tag = os.environ.get("PFTAG", "")
    return f"research_runs/{OUT_SUB}/TRIALS{tag}_{SYM}_f{fi}_{study}_t{t}.json"


def _index_key(fi, study):
    tag = os.environ.get("PFTAG", "")
    return f"research_runs/{OUT_SUB}/TRIALS{tag}_{SYM}_f{fi}_{study}_index.json"
...
def tune_A(Xst, yst, Xv, yv, spw, fi=-1):
    ...
    def ev(hp, ti=-1):
        ...
        if TRIAL_DUMP and ti >= 0:
            _put_booster(b, _trial_key(fi, "A", ti))
        return float(b.best_score), int(b.best_iteration)
    if SEARCH_MODE == "tpe":
        def obj(t):
            s, bi = ev(_space_tpe(t), t.number)
    ...
    res = parmap(lambda a: ev(a[1], a[0]), list(enumerate(des)), SOBOL_PAR)
    bi = int(np.argmax([r[0] for r in res]))
    if TRIAL_DUMP:
        _put_json([dict(trial=i, params=d, auc=r[0], bi=r[1]) for i, (d, r) in enumerate(zip(des, res))],
                  _index_key(fi, "A"))
...
def tune_B_ic(Xst, yst, wst, Xv, pdiff_v, fi=-1):
    ...
    def ev(hp, ti=-1):
        ...
        if TRIAL_DUMP and ti >= 0:
            _put_booster(b, _trial_key(fi, "B", ti))
        pv = b.predict(dv) - 0.5
        if pv.std() < 1e-9:
            return -1.0
    ...
    ics = parmap(lambda a: ev(a[1], a[0]), list(enumerate(des)), SOBOL_PAR)
    bi = int(np.argmax(ics))
    if TRIAL_DUMP:
        _put_json([dict(trial=i, params=d, ic=c) for i, (d, c) in enumerate(zip(des, ics))],
                  _index_key(fi, "B"))
