...
use ndarray_npy::{read_npy, write_npy, WriteNpyExt};
...
struct Args {
    ...
    max_samples: i64,
    /// Optional (K,) i64 .npy of EXPLICIT decision-tick indices (the `end` row of each
    /// sample, i.e. the row whose ts is the decision time). Overrides --step/--max-samples.
    /// Out-of-range indices (start<0 or end+horizon>=n_rows) are dropped.
    #[arg(long)]
    sample_ends: Option<PathBuf>,
...
fn main() -> Result<()> {
    ...
    let sample_starts: Vec<i64> = if let Some(se_path) = a.sample_ends.as_ref() {
        // Explicit decision ticks: end = idx, start = end - w + 1. Keep in-range, sorted, unique.
        let ends: Array1<i64> = read_npy(se_path).context("sample_ends npy")?;
        let mut starts: Vec<i64> = ends
            .iter()
            .map(|&e| e - w + 1)
        ...
        starts.dedup();
        eprintln!(
            "build_samples: explicit sample_ends {} -> {} valid starts (total range {})",
            ends.len(), starts.len(), total
        );
        starts
    } else {
        ...
        let s: Vec<i64> = (0..total).step_by(step as usize).collect();
        eprintln!(
            "build_samples: step={} num_samples={} (max={}, total={})",
            step, s.len(), a.max_samples, total
        );
        s
    };
    let ns = sample_starts.len();
    anyhow::ensure!(ns > 0, "no valid samples");
    ...
    // ts_paths[i, t] = depth_ts at end+1+t (ms) — wall-clock of each forward tick.
    // Consumed by grid_sim time-mode (--ts-paths) for honest time-based windows.
    ...
    let mut x_lob_writer = if a.skip_xlob {
        None
    } else {
...
fn emit_sample(
    ...
    let mut ts_path_row = vec![0i64; h];
    for k in 0..h {
        let row = end + 1 + k;
