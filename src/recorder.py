...
class Recorder:
    ...
    def record_eth_depth(self, snapshot) -> None:
        """Record ETH order book snapshot (same format as BTC depth)."""
        bids = [(float(snapshot.bids[i, 0]), float(snapshot.bids[i, 1]))
                for i in range(len(snapshot.bids)) if snapshot.bids[i, 1] > 0]
        asks = [(float(snapshot.asks[i, 0]), float(snapshot.asks[i, 1]))
                for i in range(len(snapshot.asks)) if snapshot.asks[i, 1] > 0]
    ...
    def _flush_all(self) -> None:
        ...
        if self._eth_trade_buf:
            self._write_generic(self._eth_trade_buf, self._eth_trades_dir, hour_key)
            self._eth_trade_buf = []
        if self._eth_depth_buf:
            self._write_depth_generic(self._eth_depth_buf, self._eth_depth_dir, hour_key)
            self._eth_depth_buf = []
        if self._funding_buf:
            self._write_generic(self._funding_buf, self._funding_dir, hour_key)
            self._funding_buf = []
    ...
    def _write_generic(self, rows: list[dict], directory: Path, hour_key: str) -> None:
        """Write any flat-schema buffer to parquet (trades, funding, derivatives)."""
        df = pd.DataFrame(rows)
        table = pa.Table.from_pandas(df, preserve_index=False)
        path = directory / f"{hour_key}.parquet"
        if path.exists():
            existing = pq.read_table(path)
            table = pa.concat_tables([existing, table])
        pq.write_table(table, path, compression="snappy")
        logger.debug("Flushed %d records to %s", len(rows), path.name)

    def _write_depth_generic(self, rows: list[dict], directory: Path, hour_key: str) -> None:
        """Write depth-format buffer (nested bids/asks) to parquet."""
        timestamps = [r["timestamp"] for r in rows]
        bids = [r["bids"] for r in rows]
        asks = [r["asks"] for r in rows]
        table = pa.table({
