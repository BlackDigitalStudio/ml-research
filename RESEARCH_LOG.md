...
**Last updated:** 2026-05-17 (HM6 rev3 — baseline_ref scaffolding. Pre-registered HM6 (4-sym x ~360d aligned, R1/HM5+R0, H{180,300,600}) launched on a VM via the ADC-driven phase_b_vm orchestrator (hard-cap 4h auto-DELETE). Provisional run phaseb-20260517-195819 = 4x243 [strict-intersection flaw] INGESTED but SUPERSEDED — re-confirms HM5 (R1>R0 11/12; BTC H180 R1=0.548 ric=0.048). Root cause of 360->243 diagnosed empirically (NOT a bug, NOT Cryptolake-wide): LINK-USDT-PERP genuine non-recoverable ~119d source outage 2025-12-11..2026-04-10 (raw+features absent); SOL/BTC/ETH complete. Fixes: window rule -> common-calendar-range+per-symbol days (HM6 rev2); cmd_ingest list/dict crash; baseline_360 key collision. User decision: DROP LINK, replace with LTC-USDT-PERP (HM6 rev3) — verified zero-gap in-window across all 3 streams + deepest bucket history (sequence-build extension path). Canonical set = {SOL,BTC,ETH,LTC}; corrected ~4x360 re-run launching. 2 idle VMs destroyed earlier. prior: HD1 rev19 inventory, HM5, HM3).
