...
## 28. 2026-07-15 — HX1 rev4: кросс-биржевой сигнал — первый честный OOS (43 дня)

**Контекст**: HX1 rev1–3 (июнь, дескриптивные in-sample сканы на 12–52 часах) были
застраны в ветке `claude/nostalgic-kirch-a6ad39` — перенесены в основной леджер
verbatim (d2859c0). Замороженный rev1 OOS-спек требовал ≥5–7 дней данных; накоплено
43 полных дня (инвентаризация: 8 монет × 5 источников, дыр нет). Прогнан
зарегистрированный эксперимент: train 06-02..07-01 / test 07-02..07-14, 1s-грид,
таргет Binance mid fwd log-ret, раннер `research/runtime/hx1_oos.py` (5135aef),
VM hx1-oos-001 (создана vm_provision.sh, удалена после рана), ~$1.

**Поверхность (test, rank_IC, per-day батарея)**: BASIS (дислокация цены венью,
demean 120 c) — argmax мажоры @h15: BTC pool +0.129 (13/13 дней, половины
+0.107/+0.149), BTC←Gate +0.125, ETH +0.110, SOL +0.107; монотонный спад к h60;
BNB ОТРИЦАТЕЛЕН (−0.029, 1/13) — не универсален. FLOW (ортогональный блок) на
честном окне схлопывается ~5×: лучшие пулы f15 +0.022..0.028 (LTC/BTC/BNB), f30
~+0.01, ETH/SOL ~0 — режимная гипотеза rev3 разрешена: 0.05–0.08 был горячим окном,
календарное среднее ~+0.01–0.03 и только на коротких трейлинг-окнах. Комбо
(только кросс-venue фичи; logreg — лучшая арка, GBM сид-стабилен <0.002) бьют свой
лучший компонент в ~половине ячеек, но НЕ бьют внутрибинансовый OBI(L5)-бейзлайн:
delta<0 в 95/96 ячеек (LTC h60 +0.0018 ≈ шум). Sam OBI5 OOS силён (BNB h15 +0.235,
BTC +0.196) — но он уже в span задеплоенных 74 фич.

**Вторичная аннотация (деплой-дистанция, не селекция)**: захват top-decile
конвикшена комбо-арок +0.15..+0.81 bp/событие на 15–60 c — глубоко суб-костово
standalone (мейкер-пол ~4 bp). Прогноз юзера подтверждён: направление предиктивно,
ход микроскопичен.

**Открытое → следующий rev**: единственный оставшийся путь монетизации —
аддитивность внутри 74-фичевого стека на хвостовой селекции t5/t10 (протокол
recorder-EV); prior понижен схлопыванием флоу. Ledger
`hx1oos-20260715_crossvenue_oos_43d`; артефакты
`gs://market-data-0998ac51/research_runs/hx1_oos/` (344 грид-npz + RESULT.json).

## 29. 2026-07-16 — HX1 rev5: stack-тест кросс-venue блока — аддитивности нет; годовые данные несущие

**Сетап** (замороженный пререг rev5): окно 06-28..07-14 (17 полнострумных recorder-дней,
= окно живых τ-сидов), CV11-блок (4×basis_dm + флоу-пулы 15/30/60 + 4×flow15, T−1s,
NaN→0). Машинерия: hx1_stack_{cv,rerank,days,train}.py; xsym-32 (30 vCPU по сигналу
юзера, ~2.5 ч, ~$3-4, остановлена после). Восстановленный day-путь (book/trades/fund/
liq/oi/eth → frozen July-бинарники) **бит-в-бит** с деплойными _recev-артефактами
(netl/nets dmax 0/0 на всех 33 сверенных днях).

**R-арка (главная; ре-ранкер поверх замороженного деплойного ансамбль-скора, 10 OOS
дней)**: робастного прироста НЕТ. Единственная положительная ячейка — DOGE t5:
+2.5..2.7 bp/tr на чистом скоре → **+0.4..+1.7 bp/tr под реальным джиттером 0.017,
P(Δbpd>0) 0.56–0.79, разброс GBM-сидов 4×** (= уровень сид-шума). DOGE t10 отрицательна
(−0.2..−2.4). **XRP — все ячейки сильно отрицательны** (logreg −3..−7, GBM −8..−16
bp/tr при базе +10..14, P=0.00): 12 входов на ≤16 днях переобучаются против годами
тюненного скора.

**A0/A1 (ретрейн с нуля, F71 vs F71+CV11, 2 фолда × 4 сида, Optuna 25×2)**:
**контроль сам шумовой** — ens rank-IC ~0..−0.03, EV мечется −21..+16 bp/tr со сменой
знака между фолдами, LOO-диапазоны больше средних → Δ(A1−A0) для DOGE/BTC/ETH
знак-нестабильна (шум); XRP последовательно хуже в ОБОИХ фолдах (t5 −7.8/−15.2) —
согласуется с R-аркой.

**Структурная находка (главная ценность рана)**: 17 recorder-дней НЕ обучают класс
h150 с нуля — годовые деплой-модели зарабатывают +8–14 bp на тех же днях, где
12-дневные ретрейны того же протокола дают ~0±15. Годовой масштаб данных —
несущий элемент альфы; это количественно подтверждает бутылочное горлышко
data-accrual, мотивировавшее всю программу масштабирования данных.

**Деплой-решение (вторичная аннотация)**: CV-блок в продакшн НЕ добавляется, движки
не тронуты. Условия переоткрытия: ≥3 мес полнострумного накопления / residual-тест на
живых captured-features (без ретрейна) / venue с более сильным кросс-флоу. Ledger
`hx1stack-20260716_rerank_retrain_17d`; артефакты `research_runs/hx1_stack/`.
