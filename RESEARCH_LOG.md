...
## 26. 2026-07-14 — OPS: live-vs-sim parity + реальный джиттер (DOGE, пост-апдейт окно)

**Вопрос**: с рестарта axb-exec-doge 2026-07-11 14:10:51Z (SIZE_FRAC 0.5) — совпадают ли
live-решения движка с walk-forward семантикой, и каков РЕАЛЬНЫЙ джиттер скора live-vs-sim.
Offline: замороженный recorder-EV h150 decision-путь (FB, feat71, 4-seed ансамбль, funding
anchor) на 3s-гриде; live: decision-лог axb_shadow_h150. Ledger
`parity_jitter-20260714_DOGE_postupdate`.

92 171/93 161 решений сматчено (98.9%), book-тик идентичен 99.93%, corr 0.9858,
**sd(Δscore)=0.017** (≈ уровень sd0.02 батареи, вчетверо ниже стресс-sd0.05; ячейка держит
P100 на обоих), систематики нет (+0.0005). Возле tau (score 0.8–1.0): |Δ| p50 0.022,
хвост p90 0.10 — тяжелее гаусса, источник = тик-десинк потоков в бёрстах (0.07% точек).
**Сделки: 5/6 воспроизведены точно** (|Δ| 7e-4..1.3e-2, сторона/тик/селекция совпали);
6-я (0714 15:31:57) — 12-секундная ДЫРА ЗАПИСИ recorder-книги (offline на протухшем тике,
live видел реальный бёрст) — артефакт доступности данных, не отклонение движка. Флаги возле
порога: both 17 / live-only 3 / offline-only 6 — все в бёрстах; busy-lock исполнителя делает
epizod-level наборы сделок идентичными. Вердикт: live-путь консистентен, реальный джиттер
внутри доказанного конверта робастности.

**Инцидент (найден и купирован)**: chronos-gcs-sync падал с 2026-07-12 07Z
(FileNotFoundError — гонка с retention prune), raw-выгрузка стояла ~2.5 суток при
RETENTION_DAYS=3 — до удаления незалитых данных оставались часы. Ручной one-shot sync
(19:34–19:49Z) закрыл бэклог. Корневой фикс юнита — отдельным изменением по согласованию.

## 27. 2026-07-15 — HD5-DEPLOY: FIXQ в проде (DOGE t10 PASS+LIVE, XRP t5 FAIL гейта) + движок v2

**DOGE**: гейт `fixq_recev-20260714_DOGE_deploygate` (+T10-амендмент юзера) — PASS 4/4 на
TEST 0708–0714: FIXQ t10 +9.28bp/tr (bpd +68.9 ≥ +67.2 DYN), LOO 0/7, jitter P100 на
sd 0.017 (реально замеренный уровень) / 0.02 / 0.05. Parity-ритуал: 3/3 пересчитанных дня
бит-в-бит против `_recev_h150anch_DOGE`. ЗАДЕПЛОЕНО 2026-07-15 00:31:47 UTC:
axb_engine_v2 (sha256 806e17172cfae943; FREEZE_TAU + захват фич по решению —
features/{day}.bin, grid_ts i64 + 71×f32), TRADE_BUDGET 5→10 в обоих юнитах,
RECEV_DIR → `_recev_h150anch2_DOGE` (17 дней, 485 078 скоров), SIZE_FRAC 0.5 без
изменений. Frozen taus [.863464 / .817010 / .774477 / .728887] — бит-в-бит с offline-
ожиданием; решения и захват фич проверены вживую. Витрина trading_algorithm обновлена
тем же окном (манифест + юниты + gate-снапшот).

**XRP**: гейт `fixq_recev-20260714_XRP_deploygate` t5 — FAIL по условию (2): bpd FIXQ
+12.8 < DYN +20.9 (4 тест-дня, разница = одна пропущенная сделка 0714 при замороженном
τ 0.9296). Остальные условия PASS, включая jitter на реальном уровне 0.017 (P100) —
годовая sd.05-хрупкость ячейки на recorder-frame НЕ воспроизводится. По пререгу политика
НЕ меняется (динамический causal t5 остаётся); движок v2 выкачен только с захватом фич
(frozen=false, rolling-state цел). Open item: пере-гейт на длинном окне.

**Джиттер-инженерия** (директива юзера: свести к бит-в-бит): корень хвостов — WS-
реконнекты рекордера (несколько раз в день; пример 0714 15:32:01–08 = та самая 12s-дыра).
Захват фич движком v2 делает будущий реплей бит-экзактным по построению. Рекордер:
...
## 28. 2026-07-16 — HD3 rev10/rev11: честные BTC/ETH ячейки + протокол-v2 бридж

**rev10 (ledger `tb3s-20260715_h150d_btc_eth_dense_rebuild`).** Пересборка BTC/ETH на
**H_TICKS=5100** (полное покрытие 510s = 60s вход + 150s холд + 300s chase; @100ms поток
≤10 тиков/с ⇒ граница точная, замеренный max need = 5006; parity-день бит-в-бит 9/9
массивов) × {true, anchored} funding × 4 сида × 6 фолдов, t5, протокол rev8.

| ячейка | per-seed | ENS (n, hit) | LOFO min | jitter .02/.05 |
|---|---|---|---|---|
| **BTC × TRUE** | **+9.95±6.32** 4/4 | **+12.27** (449, 68.8%) все 6 фолдов + | +9.74 | **P100** / P28 |
| BTC × ANCH | +0.88±2.33 | +4.88 (511) | +1.60 | P80 / P1 |
| **ETH × ANCH** | **+5.97±4.47** 4/4 | **+10.16** (448, 58.3%) | +5.58 | P65 / P9 |
| ETH × TRUE | +2.95±4.31 | +2.03 (507) | −0.19 | P38 / P13 |

Чтение поверхности: (1) **фандинг-семантика символ-условна с противоположными знаками**
(BTC хочет true, ETH хочет anchored) — анкер DOGE/XRP не универсален; (2) **старый
BTC-anch (+6.20 на H1800) был в основном артефактом touch-mark** на коротком пути (рухнул
до +0.88), тогда как BTC-true сохранил среднее (+9.69→+9.95) при sd ×3.5; (3) ансамбль-
усиление вернулось на обоих argmax-вариантах. Гейты: sd.05 не прошёл никто (BTC-true P28,
ср. XRP-override P41); деплой-кандидатуры не объявлены, путь BTC-true = recorder-EV
кросс-чек (BTC recorder-стримы инвентаризировать). Ops-инцидент: 16×14GB = 96% RAM убил
гостевую сеть VM (~8h потеряно) → RAM-клэмп 75% + swap + timeout-вотчеры
(KNOWN_PITFALLS, orchestrate2).

**rev11 (протокол-v2 бридж + изоляция).** `subs60_xgb_sobol_v2.py` = семантика v1, но
Sobol-дизайн вместо последовательного TPE + QuantileDMatrix/inplace_predict + RAM-диета
(−6GB/джоб). Бридж BTC×true×4 сида: **ансамбль +12.59 vs +12.27, Spearman фолдов 1.000,
ΔAUC ≤0.001**; скорость **×2.4–2.7/джоб + ×1.9 упаковка** (кампания ≈ ×4.5–5);
FOLD_PAR=6 бит-в-бит 66/66. Единственный критерий-FAIL — нижняя санити-граница сид-sd
(1.80 < 1.90), в сторону улучшения. Изоляция (SEARCH_MODE=tpe, сиды 0/2): TPE-режим v2
воспроизвёл v1 **в точности** (+1.10/+15.01, пофолдово идентично) ⇒ вся разница v1↔v2 =
дизайн поиска; сид-разброс v1 был доминирован TPE-лотереей траектории. **Адопция v2 =
решение юзера** (предложена с оговорками: v2 seed-sd несравнима с v1; сид-гейты
пере-калибровать; ens-уровень стабилен).

**s28 addendum (2026-07-16 07:40 UTC): протокол v2 АДОПТИРОВАН** решением юзера. Frozen:
`subs60_xgb_sobol_v2.py` md5 a751ec3fef8ff32302fbf80af9be6481 — дефолт для новых тиров
(v1 остаётся для v1-сравнимых ре-ранов); правила: v2-ячейки тегируются, seed-sd между
протоколами не сравнивается (сид-гейты пере-калибровать), SEARCH_MODE=tpe только для
изоляций; orchestrate2 XSYM_TRAINER. Ledger `xgb-sobol-v2-ADOPTION`.

**s28 addendum-2 (2026-07-16 08:10 UTC): пертурбационный гейт переопределён** (директива
юзера): биндинг-уровень = sd 0.02 (покрывает измеренный реальный джиттер 0.0171 с запасом;
captured-features реплей движка v2 и FIXQ убирают сам источник), sd 0.05 — только
диагностика. Пере-аннотация rev10: **BTC×true — формальный деплой-кандидат** (оба
вторичных гейта PASS: сид-гейт +3.63, jitter sd.02 P100); остаток цепочки до капитала =
инвентаризация BTC recorder-стримов → recorder-EV кросс-чек → per-symbol funding mode в
...
## 30. 2026-07-18 — HD3 rev19: расширение на BNB/LTC/SOL — честная пересборка × фандинг × no-ToD (поверхность)

**Вопрос**: переносится ли цепочка улучшений BTC/ETH (честная сборка H5100 → фандинг-
семантика → ToD-абляция) на оставшиеся CL-символы (LINK исключён: 119д дыра). Prereg
`tb3s-20260718_bnb_ltc_sol_honest_rebuild_PREREG`; протокол v2.1 (2a435f32), 4 сида, t5,
батареи OPS-GATES rev2; 365д/символ, N~9M, EXC=0.

**ПОВЕРХНОСТЬ (ENS t5 | per-seed ±sd | BOOT P(EV>0) | jitter sd.02)**:

| символ | anch | true | true×no-ToD | argmax |
|---|---|---|---|---|
| BNB | −3.44 (P27) | −0.34 (−0.14±3.48, P54) | **+2.51** (625tr, hit 56.6%, CI90 [−3.3,+9.1] P77, LOFO min −1.77, jit P36) | true×no-ToD |
| LTC | −6.14 (P20) | **+1.21** (529tr, +1.23±4.60, P64, jit P5) | −4.09 (P30) | true |
| SOL | −5.41 (P17) | **−0.11** (691tr, P50, jit P16) | −0.98 (P46, per-seed +0.58±3.34) | true |

**Условия, движущие поверхность**:
1. **TRUE-фандинг — argmax на всех трёх** (Δ vs anch +3.1/+7.4/+5.3 ENS): BTC-паттерн
   распространяется на все extension-символы; деплойный якорь DOGE/XRP вне DOGE/XRP —
   неверная семантика. Live funding_rate (col13) — топ-колонка Bg на всех трёх (8.3–8.9% gain).
2. **Ответ на ToD-абляцию символ-условен**: BNB +2.9 ENS (hit 47.9→56.6), SOL ~0 (per-seed
   +6.3), LTC −5.3 — удвоение ETH НЕ обобщается, хотя все три модели клоко-доминированы
   ETH-класса (ToD 24–27% Bg gain; 24 fold-модели/символ из v2.1-дампов). Доля ToD в gain
   НЕ предсказывает знак ответа на абляцию.
3. **Негативы rev8 — не артефакт горизонта**: плотности книг ~1.5/с ⇒ H1800 покрывал
   ~1200s ≥ нужных 510s; anch-ячейки при матче семантики сдвинулись мало (BNB −3.00→−3.44,
   SOL −5.05→−5.41; LTC −10.77→−6.14 смешивает H5100 и протокол v1→v2 — только conclusion-level).
   Прирост тира дал фандинг (+ToD для BNB).
4. **Магнитуды**: argmax-ячейки +2.5/+1.2/−0.1 против +12..+16 у деплой-класса — h150-класс
   политики собирает на BNB/LTC/SOL в 5–10 раз меньше при всех измеренных условиях.

**Secondary (деплой-гейт, отдельным полем)**: биндинг-гейт не проходит никто (лучшие: BOOT
P77 BNB-notod < 90; jitter.02 P36–47; сид-гейт mean−sd < 0 везде) — деплой-кандидатов нет,
капитальных действий нет.

**Анатомия** (Bg, 24 fold-модели/символ): ToD 24–27%, vol 11–14%, book/OBI 10–13%, liq
9–11%, funding 9–10%, OI 6.5–8.5%, btc_lead 1–2% — «clock+carry» модели, как ETH до абляции.
**Ops**: фиксы orchestrate (ANCH_SRC/DST passthrough — пойман до запуска: дефолты перезаписали
бы rev8 h150anch; orchestrate2 XSYM_OUT_SUB). VM xsym-32 ~12.6ч ≈ $25, остановлена 22:33Z.
Артефакты: датасеты h150d{,anch} ×3, 36 SEED json, 216 PERFOLD, **576 model-дампов**, 9 ENS,
rev19_logs.tgz. **Открытые ходы** (решение юзера): rev14/15-рычаги на BNB-notod (8 сидов,
tau-floor/FIXQ), hold-sweep 90/240, либо зафиксировать поверхность и остановить расширение.

## 31. 2026-07-19 — HD3 rev20: BNB глубокое погружение — сиды 4–7, селективные формы, harmony (джиттер отменён юзером)

**База**: BNB × true × no-ToD (argmax rev19). Prereg `bnb_levers-20260719_PREREG`; 163 тест-дня,
BOOT L=7×1000; джиттер-батареи ОТМЕНЕНЫ директивой юзера (экономия времени).

**L1 сиды 4–7**: per-seed [+0.2/−0.7/−2.1/−2.6] ≈ сиды 0–3; собственный ens **+0.12** — знак
совпал, но <50% от +2.51 ⇒ **draw-confirmation FAIL**: магнитуда rev19 была чувствительна к
дро; честный уровень = 8-сидовый DYN **+2.09** (740tr, bpd +9.51, P75). **Пул-сид-гейт −2.37
FAIL** — per-seed в среднем ОТРИЦАТЕЛЕН: вся положительная экономика BNB живёт в ансамблевом
слое (более сильная форма DOGE-эффекта).

**L2 селективная поверхность (главная находка)**: рабочая форма BNB — **КОНСЕНСУС**, не флор
(ETH-флор переносится отрицательно: 0.97–0.99 все минус; tau-floor ≤ +1.9). Монотонно по k:
≥4/8 −0.11 → ≥5/8 +1.33 → ≥6/8 +1.58 → **≥7/8 +5.91** (270tr, bpd +9.79 ≈ полный DYN-bpd,
hit 56.3%, LOFO(mo) +4.20, CI90 [−1.45,+17.72] **P90**, **0/6 отриц. месяцев**) → **8/8
+10.05** (138tr, bpd +8.51, hit 59.4%, LOFO +6.63, CI90 [−0.76,+26.06] **P94**). Тренд, не
спайк; флаг multiple-testing по k несём.

**L3 harmony (дизъюнктный, rev18-дизайн)**: направление реплицируется в ОБЕ стороны —
select 4-7/grade 0-3: гармония +1.70 vs разногласие −0.92; select 0-3/grade 4-7: +4.55 vs
+0.25 — независимая структурная поддержка консенсус-механизма.

**Чтение**: у BNB есть реальный, структурно подтверждённый, консенсус-концентрированный
maker-эдж; потолок экономики в этом классе политики ≈ **bpd +9.8 ≈ +1.5–1.6%/мес @0.5** —
в 5–8 раз ниже задеплоенных ног (DOGE +74.9 / XRP +52.4 / ETH +42.7 bpd). **Secondary
(деплой)**: consensus ≥7/8 на планке BOOT P90, 0 отриц. месяцев, но сид-гейт FAIL и экономика
маргинальна — капитал НЕ предлагается; статус = охарактеризованная резервная ячейка.
Продолжения (решение юзера): recorder-EV кросс-чек консенсус-формы как 5-й микро-ноги /
hold-sweep 90/240 / паркинг. Ops: 4 джоба ~2.1ч ≈ $6, xsym-32 остановлена; артефакты
(SEED4-7 + 48 PERFOLD + 96 model-дампов) в _v2notod/.

## 32. 2026-07-19 — HD3 rev21: BNB — ширина вол-гейта A (топ-5% → топ-10%), NF10-вариант

**Вопрос юзера**: те же consensus-ячейки, но модель A ловит топ-10% волатильных движений.
Вариант-тренер byte-copy 6fb0171e (NF_RATE 0.10 + GATE_PCT 10 — одна концептуальная ручка,
зафиксировано в prereg `bnb_nf10-20260719_PREREG`), 8 сидов, без джиттера.

| ячейка | NF5 (rev20) | NF10 (rev21) |
|---|---|---|
| per-seed mean | −1.31 | −2.14 |
| DYN 8-seed | +2.09 / bpd +9.51 / P75 | +0.62 / +2.93 / P64 |
| CONS ≥7/8 | +5.91 / +9.79 / WR 56.3 / Sh 1.52 / P90 | +2.19 / +3.58 / 52.8 / 1.05 / P80 |
| CONS 8/8 | +10.05 / +8.51 / 59.4 / 1.71 / CI90 [−0.8,+26.1] P94 | +6.69 / +6.24 / 57.2 / **2.13** / **CI90 [+3.7,+13.8] P100** |

**Чтения**: (1) расширение вол-зоны до топ-10% **разбавляет эдж** — сигнал BNB сконцентрирован
в топ-5% вол-режиме; прережённое направление успеха (bpd/Sharpe вверх) провалено по bpd везде.
(2) Консенсус-лестница остаётся монотонной и под NF10 — механизм робастен к ширине гейта,
уровень — нет. (3) Единственный выигрыш NF10: ячейка 8/8 стала **глаже** — Sharpe 1.71→2.13,
первая BNB-ячейка с полностью положительным BOOT CI90 (низ +3.70, P100) — ценой −27% bpd и
опоры на январь (LOFO(mo) +1.70 vs +6.63). **Secondary**: капитального кейса нет (bpd +6.24 ≈
+1%/мес @0.5); резервный статус без изменений; NF5-консенсус — лучшая экономика BNB,
NF10-8/8 — самая гладкая форма. Ops: 8 джобов ~2.6ч ≈ $7, xsym-32 остановлена; артефакты
в `_v2notod_nf10/` (+192 model-дампа).

## 33. 2026-07-19/20 — HD3 rev22+rev23: BNB — бюджетная поверхность, режимная анатомия, liq/OI-абляции

**rev22 (бесплатный тир, re-анализ артефактов)**: (1) поверхность бюджета селективности
МОНОТОННА к хвосту: DYN t10 −1.94 → t5 +2.09 → t2.5 +4.88 → **t1 +12.79** (159tr, WR 64.8%,
LOFO(mo) +9.06, BOOT CI90 [+5.69,+19.16] P99, Sharpe 2.94, maxDD −1.17%, 0 отриц. месяцев);
**дро-репликация в полной магнитуде** (сиды 0–3: +12.12; сиды 4–7: +8.95; bpd ~+11–12 оба) —
первый такой рычаг BNB; консенсус ретроспективно был прокси этой же хвостовой концентрации.
Занятость FIXQ-подобная: 159 сделок в 14 дней. (2) Анатомия режима: t1-дни = бёрсты rv+
(+0.5σ) с положительным фандингом (+0.35σ); убытки на самых экстремальных вол-днях; EV
монотонен по liq-интенсивности дня (Q1 +8.5 → Q4 +29); **январь — carry-режим, не вол**
(rv −0.5σ, funding +0.5σ). Ledger `bnb_budget_regime-20260719`.

**rev23 (абляции, 8 джобов ~$7)**: обе семьи — НАСТОЯЩИЙ сигнал, ETH-сценарий «шумного
костыля» опровергнут: no-LIQ деградирует всё (per-seed −1.88, t5 −0.60, t1 +7.88);
**no-OI СХЛОПЫВАЕТ хвостовой эдж** (t1 +12.12 → +0.45) — OI-дельты (кол. 59–60) — несущий
носитель t1-сигнала. Механизм BNB охарактеризован: **positioning-burst феномен** (бёрсты
OI-дельт + liq-интенсивность + положительный carry), собираемый на экстремальной
селективности; разбавление базы при t5 — сделки вне бёрстов, фичевой абляцией не лечится.
Ledger `bnb_liq_oi_ablation-20260719`. Пути к статистической мощности (решение юзера):
hold-sweep 90/240 / прережённое режим-кондиционирование по OI/liq-активности (семья,
провалившая OOS у ETH — нужен строгий out-of-selection дизайн) / накопление live-данных /
принять DYN t1 как терминальную годовую форму.

## 34. 2026-08-06 — HBV1: бесплатный Bybit-архив как замена CL — DOGE anchored-h150 реплика (Modal)

GCS умер вместе с GCP-биллингом (2026-08-05). Вопрос: воспроизводится ли задеплоенная
DOGE-ячейка на БЕСПЛАТНЫХ архивах Bybit (quote-saver ob500/ob200 L2 + public.bybit.com
trades), при байт-неизменном frozen-слое (gcs-шим -> Modal Volume, virginship07, $3.70).

Ячейка: DOGE × 390d Bybit-linear 2025-05-09..2026-06-02 × anchored h150 × v2 × 7 фолдов.
Поверхность: PER-SEED t5 +9.15±6.28 [4.5/8.2/5.6/18.3] 4/4>0, t10 +6.19±4.26 — класс CL
воспроизведён (CL v1: +8.14±2.55 / +4.64±0.93); fold2-концентрация (зимний бёрст)
воспроизводится кросс-венью. ANSAMBLE t5 +5.06bp (1111tr, hit 53.5%), все LOFO>0,
jitter sd.02 P100%; ENS t10 −0.56. Вторичная аннотация: ансамблевое УСИЛЕНИЕ не
переносится (CL 1.64×, Bybit 0.55×) — гипотезы: нет ликвидаций (в CL были реальные),
OI 5min vs 4s, venue-микроструктура, v2/7-фолдов. Датасет/билд: 390 дней без гэпов,
fill 0.82-0.90, книга ~9.7/с, funding/OI НАСТОЯЩИЕ Bybit (REST жив с Modal-егресса).
Ledger: hbv1-20260806_bybit_doge_h150anch_repl (+PREREG, +AMEND1).

## 35. 2026-08-06 — HBV1 rev2-4: атрибуция ансамблевого разрыва, no-OI, union — Bybit-ячейка сошлась с CL

Три параллельных прогона на Modal ($8.34 суммарно с rev1):
- **rev2 (v1-протокол, тот же датасет)**: ENS t5 **+12.99bp** (1052tr, 5.6/д) = 97% от
  CL-цели +13.35 — ансамблевый разрыв rev1 был ПРОТОКОЛЬНЫМ (TPE-разнообразие сидов;
  ens/per-seed 0.55→0.91). Бесплатные Bybit-данные воспроизводят задеплоенную ячейку.
- **rev3 (no-OI, DROP 59,60)**: 5-мин OI-прокси был ШУМОМ — per-seed t5 +9.15→+14.40
  (+57%), вся бюджетная кривая выше; DOGE-Bybit противоположен BNB rev23. Для будущих
  Bybit-ячеек: исключать 59-60, пока OI 5-мин.
- **rev4-репликация (union на v1-сидах)**: union > mean-rank на обоих независимых дро;
  лучшие формы: T1.25 +30.42 @3.6/д (BOOT P100), T2.5 +23.84 @6.5/д (P99). Величина
  по-прежнему в зимнем бёрсте (fold2). Несмежная комбинация no-OI×union не гонялась.
Ledger: hbv1-...V1PROTO / _nooi_ablation / _union_policy_V1REPLICATION.

## 36. 2026-08-06 — HBV1 rev5-6: ликвидации закрыты, зеркало HF, no-OI×union — лучшая Bybit-форма

rev5: liq-семейство для исторических Bybit-ячеек НЕДОСТУПНО из бесплатных источников
(Binance-архив удалён, Bybit не публиковал, Coinalyze free ~2.5 мес суб-дневной глубины,
Tardis free 1 день/мес) — cols 19/43/56-58 нулевые, это потолок. Cron+бэкфилл Coinalyze
остановлены юзером. Артефакт-ядро (9.32 GB) зеркалировано в приватный HF
delmiron27/hbv1-bybit-artifacts (Modal+HF+GitHub = независимость от смерти аккаунта).
rev6: **no-OI × union складываются**: T1.25 +24.19bp @3.3/д (7/7 фолдов, BOOT floor
+7.81 — сильнейший из измеренных), T5 +14.69 @9.7/д (floor +2.48, впервые
положительный floor на полном tpd). Структура здоровее: fold2 ~50% суммы (было 79%),
консенсус-инверсия — k=3/4 лучшие вёдра (+37/+34), k=2-мусор исчез. Без грубого OI
сиды согласны на ХОРОШИХ сделках. Ledger: hbv1-20260806_nooi_x_union.

## 37. 2026-08-07 — HBV1 rev7/9/11: гейт валидности, честные комиссии, 8-сидовые пулы — итоговая форма

Гейт (директива: без отрицательных фолдов / без концентрации в одном периоде / BOOT
floor > 0) + честные Bybit-комиссии (non-VIP maker 4bp/круг; кросс-venue исполнение на
Binance неизмеримо — bookTicker-архив снят после 2024-04) + стандарт отчётности
(ROI/maxDD при капитале 1.0, батарея policy_metrics для ЛЮБОЙ формы). Итог программы:
**v1-nooi8 union T0.3125 — net +31.5bp/tr @1.4/д, hit 63.7%, ROI +14.2%/мес,
maxDD −6.5%, worst-day −2.8%, Sharpe 3.6, LOFOmin +19.1, 0 отриц. фолдов, BOOT floor
+15.9, P100** — лучшая измеренная форма HBV1. 8 сидов против 4 покупают не EV, а
гейт-устойчивость (v2-nooi8 gross проходит гейт в 3 точках против 1). Комиссии убивают
все mean-rank ансамбли и union T>=2.5. HP-оптимума не существует (rev10): argmax по
val-IC ~некоррелирован с реализованным EV — HP-поиск это генератор декорреляции.
Расход кампании ~$21.7/$30. Ledger: hbv1-20260807_8seed_nooi_unions (+rev9/10/11).

**Поправка 2026-08-07 к s34-s37:** GCP ПРИОСТАНОВЛЕН, не потерян (формулировки «умер/
потерян» в s34/s36 неточны). CL-архив 585GB (вкл. raw/liquidations за всё окно),
recorder-данные (вкл. USDC-книгу) и GCS-артефакты предположительно восстановимы при
возобновлении биллинга; «невозможные» измерения rev5/rev12 станут возможными. При
реактивации — оперативно зеркалировать критические префиксы (ретеншн-окна провайдера).
Ledger: hbv1-20260807_gcp_suspended_CORRECTION.

## 38. 2026-08-07 — HBV1 rev14-16: консенсус K-из-N, декорреляционные оси, RF-рецепт — новый лучший пул

Два пользовательских вопроса, обе оси прошиты за одну сессию (~$5-7 Modal virginship07):

**rev14 (консенсус K-из-N, analysis-only)**: сделка засчитывается когда ≥K членов
независимо перешагнули СВОЙ causal-порог при цели T_s (K=1 ≡ union). Лестница K
монотонна по качеству (hit↑, DD↓) на всех пулах, но на seed-only пулах гейт не
проходит (1-3 отриц. фолда всюду; согласие клонов не селективно). На v2-nooi8
консенсус бьёт свой union при равном tpd: cons T1.25 k≥3 +27.1 @1.9/д vs +21.6.

**rev15 (оси шире сида: фич-подпространства + hold-горизонт)**: Jaccard торгуемых
хвостов 0.413 (сиды) → 0.285 (+25%-бэги фич). fb-члены индивидуально не хуже сидов
(среднее +16.2 vs +14.4). Чистый пул fb4: cons T2.5 k≥3 **+52.1bp @1.1/д, ROI
+18.7%/мес, DD −6.7%, floor +17.3, PASS** — EV/tr выше чемпиона rev7 на 65%.

**rev16 (RF-рецепт: + day-level бэггинг трейна, тренер v2.2 BAG_FRAC)**: бэг 0.632
не только декоррелирует (Jaccard 0.191, side-agree 93.5%), но УЛУЧШАЕТ членов
(rf-среднее +28.2 — сильнейшие одиночные члены ячейки; согласуется с subsample-
тенденцией rev10). Пул rf4 — лучшая поверхность программы (fee-honest): **union
T0.625 +46.5bp @2.6/д, ROI +42.1%/мес, maxDD −17.2%, LOFOmin +30.6, PASS**; union
T0.3125 +50.3 @1.7/д; cons T10 k≥4 +72.4 @0.4/д hit 86.7%. Механизм связки:
консенсус окупается пропорционально декорреляции членов — на rf/fb пулах
консенсус-лестница даёт PASS-угол, недоступный union.

Оговорки (обязательны при переиспользовании): пулы = единичные дро конфигураций
(нужна репликация с новым RNG бэгов до деплой-клеймов); high-K углы с n≤26 —
анекдоты; grid-селекция давит даже через гейт; ячейка DOGE×390d×Bybit×anch-h150×4bp.
Ledger: hbv1-20260807_consensus_kofn / _diversity_axes / _rf_recipe (+3 PREREG).

## 39. 2026-08-07/08 — HBV1 rev18-21 + HBV2: sized-union, портфель, лес-32, два новых символа, канон signal-first

Продолжение s38 одной сессией (~$55 из $120 по 4 аккаунтам; DD-25 нормировка — rev21).

**DOGE (rev18/19)**: sized-union w=(k/N)^γ — гладкий регулятор экспозиция↔качество,
bpd-floor максимален у рыхлых γ=0 форм (rf4 T2.5: floor +51bp/д). Кросс-семейные
корреляции форм 0.10-0.19 (режимы комплементарны: rf — вол/отриц.фандинг,
fbag-cons — плоский/полож.фандинг) → invvol-портфель 7 форм: F10=3.33,
+62.3%/мес, p10 +28.1%, Sh 3.28. Лес 32 членов: union разбавляется с N, а
**консенсус доминирует ячейку — rf32 cons T2.5 k≥8: +60bp @2.1/д, hit 71.8%,
floor +18.1; hit до 91% в строгом углу; rf8 cons T5 k≥4 в DD-25: +73.7%/мес @F1.8,
p10 +22.8%** — лучшие формы программы. Дро-репликация: 8 непересекающихся
4-членных пулов, sd ±17.9bp — мелкие пулы лотерейны, большой консенсус усредняет.

**HBV2 (1000PEPE, LTC; сборки 390/390 без гэпов)**: транспланта рецепта без
настройки осей не существует — подтверждён канон юзера (rev3, стоящий):
СНАЧАЛА одиночная модель на своих осях символа, ПОТОМ ансамбль. Форензика
членов (rev20; случайные бэги = эксперимент ценности фич): DOGE — momentum_60s
токсичен; LTC — funding несущий, ToD токсичен (L3 noToD: соло +12.4 = 2× базы);
PEPE — бета к btc_ret60 + sin_f8 сезонность несущие, собственный флоу токсичен
(P3: соло +31.8 = 2× базы, дроп работает ТОЛЬКО на h240). Целевые ансамбли
(оси фиксированы, декорреляция сид×дата-бэг): LTC — первые PASS-формы символа
(cons T2.5 k≥4 +39bp, ROI25 +89%@F11 p10 +12.7%); PEPE — полярность консенсуса
РАЗВЕРНУЛАСЬ (k≥5 +73.3, hit 94.4%, PASS; n=17-36 — анекдоты, F10-клэмп помечен).

Стоящие поправки процесса: rev21 — сравнение форм только при общей просадке 25%
(battery считает F10/ROI25/p10 для каждой формы); solo→ансамбль канон (rev3 HBV2);
инкрементальные сейвы аудитов; corr-fn 16GB. Ledger: hbv1 rev18/19/20/21,
hbv2 rev1-4. Артефакты зеркалятся на HF со всех 4 аккаунтов.

## s40. 2026-08-08 (вечер): масштаб членов открывает пол выборки; XRP — 4-я ячейка (HBV2 rev5/6, HBV3 rev1-3)

Задача сессии: довести 1000PEPE/LTC/XRP до гейтов и p80 при DD-25 (директива:
сравнение только при MaxDD25; бюджет $30/аккаунт, virginship07 исключён; XRP
качать с нуля — «докачать» = скачать из архива Bybit).

**HBV2 rev5 (масштаб N 6→16 на фиксированных осях, ~$12)**: гипотеза rev4
подтверждена — консенсусное разрешение растёт с N и строгие углы пересекают
пол выборки. **1000PEPE P3-16: 8 PASS-форм, ПЯТЬ p80** (первые p80 символа):
T10 k≥12 — n=164, hit 81%, **ROI25 +117.4%/мес @F7.6, p10 +26.5%**; T2.5 k≥8 —
n=118, hit 85.6%, EV +51.5, ROI25 +39.8%@F3.6 (консервативная). Строгий гребень
P3-16 не торгует бёрст-фолд f2 (6/7) — противоположность DOGE. **LTC L3-16:
2 PASS** — k≥8/16: n=216 (было 104), EV +52.7, **ROI25 +95.0%/мес @F3.91**
(F10 из нереальных 11 стал операционным), p10 +5.7%. p80 на LTC этим окном не
закрыть: SE≈37 против нужных ≤21 → n_min≈660; ступени k≥9+ гибнут на покрытии
4/7 фолдов. Физика разная: PEPE несёт эдж точностью (hit 72-86%), LTC — размером
выигрыша (hit 58-65%).

**HBV2 rev6 (горизонты/оси, ~$1.5)**: LTC noToD платит ТОЛЬКО h150 (5.5/12.4/7.9
на h90/h150/h240). **PEPE P5 = P3-дропы + momentum_60s: соло +41.2 — бьёт P3 +31.8;
токсин momentum с DOGE переносится** (3-й символ). Но пул P5-10 (N=10) слабее
P3-16: разрешение лестницы и разнообразие важнее силы оси; P5 — вторая
независимая ось (6 upow-форм, кандидат в портфель).

**HBV3 rev1-3 (XRP, tg-bot-on)**: архив 2-2.5× веса DOGE (248MB/д на старте окна),
390/390 дней без гэпов, anch XRP.npz 2258MB. Соло-поверхность: **на сырых осях
XRP сигнала НЕТ** (X0 −0.3 против PEPE +15.4 / LTC +6.0 / DOGE ~+14); аргмакс
X4 = дроп собственного потока @h150 +7.31 (ось PEPE переносится, но на
ПРОТИВОПОЛОЖНОМ горизонте — h240 на XRP токсичен всюду); momentum-дроп +4.75,
noToD +3.17. Комбинации осей НЕ аддитивны: X6 (flow+momentum) +6.25 < X4;
X7 (+noToD) −1.35 — ломает модель (перенос P5-приёма на XRP не сработал —
правило ячеек). XRP — слабейшая соло-ячейка четвёрки; пул 6 членов на осях X4:
0 PASS — гребень k≥3 (T2.5: n=170, EV +46.5, ROI25 +29.4%@F2.1) блокирован одним
отрицательным фолдом, середина лестницы немонотонна. Масштаб 6→16 (реле датасета
на ksagachev, PERFOLD-ы обратно) прецедент LTC НЕ повторил: 0 PASS из 64, чистые
углы только при n≤37 (T5 k≥11: hit 94.6%, EV +32.8). **XRP закрыт по
предрегистрированному правилу rev4 — «не доводится рациональными усилиями»**;
затрачено ~$21-27. По правилу юзера программа перешла на скринированный лёгкий
волатильный символ: HBV4 = 1000BONK (~50% веса DOGE, мемкоин-класс, blackdigital-kz),
скрин-таблица кандидатов в rev4-записи (1000FLOKI 23% — следующий в очереди).

**HBV3 rev5 (повторное вскрытие XRP по директиве, ~$3)**: (A) гетеро-осевой пул
из 8 solo-армов — члены с разных осей голосуют в противоположные стороны,
majority-side инвертируется (hit 26-46%) — прецедент DOGE-пула H не расширяется
с горизонт-селекторов на оси признаков; (B) **прямая репликация задеплоенной
формы README-4.1: эджа XRP на Bybit НЕ СУЩЕСТВУЕТ** — сиды +0.27±2.39 (2/4
против 4/4 на CL), ансамбль −1.75bp/861сд (CL: +12.68/504), все LOFO<0, джиттер
sd.02 P(EV>0)=4%; на t1/t2.5 хуже (хвост анти-селектирован). Пара кросс-венью
вердиктов: DOGE перенёсся (фундамент HBV1), XRP — нет: эдж 4.1 был специфичен
Binance-книге/эпохе; (C) X8 h90 +1.79 — h150 подтверждён горизонтным максимумом
оси X4. Все три арма отрицательны → XRP закрыт повторно на обоих семействах форм.

**HBV4 (1000BONK — замена XRP, blackdigital-kz, ~$10 при капе $15)**: сборка
390/390 (~50% веса DOGE); соло-сюрприз — BONK не PEPE-класс: оси PEPE токсичны
(−13…−19bp), аргмакс B4 = noToD (ось LTC) +9.08. Пул 10 членов: **2 PASS-формы,
одна p80 с первой попытки** — cons T10 k≥7: n=171, EV +30.8, hit 69.6%, ROI25
+15.5%/мес @F1.74, p10 +5.5%. 4-й валидный символ программы. Карта осей по пяти
символам: несущие у каждого свои, классового закона нет.

Ops: gRPC Modal через egress-прокси лечится modal[api-proxy-support] + CA в
certifi; вырожденный угол бутстрепа (все сделки на недостижимых сэмплеру
хвостовых днях) ронял аудит целиком — guard в battery (ec31347), непустые формы
байт-не тронуты; между окончанием тренировки членов и стартом аудита держать
≥1 мин (гонка видимости тома). Леджер: hbv1 rev20 (просроченная measured-запись
форензики закрыта по артефактам), hbv2 rev5/6, hbv3 rev1/2/3. Зеркала HF —
5 аккаунтов (добавлен tg-bot-on/XRP).
