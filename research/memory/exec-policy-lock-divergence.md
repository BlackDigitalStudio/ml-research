...
**3/19 live-промаха входа при sim-филле всех** (rev14: ×0.6 ОТОЗВАН как оценка —
бутстрэп 90% CI на отношении live/sim = [0.15, 0.92] на пуле и [-0.39, 0.90] по
DOGE; пул был завышен XRP/ETH без промахов; биржа не берёт ничего — комиссия 0,
фандинг 0; shortfall = несостоявшиеся сделки ~2/3 + дрейф PnL ~1/3). Остаётся
только направление: sim-PnL — ВЕРХНЯЯ ГРАНИЦА, величина не измерена. Мониторинг: еженедельный
